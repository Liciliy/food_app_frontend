      ...
      >
        <div className="flex items-center space-x-3">
          <span className={`px-2 py-1 rounded-full text-xs font-medium ${style.bg} ${style.text}`}>
            {style.icon} {capitalize(meal.meal_type || 'unknown')}
          </span>
          ...
          {meal.food_items && meal.food_items.length > 0 && (
            <div>
              <div className="flex items-center text-xs text-gray-500 mb-2">
                ...
                Food Items ({meal.food_items.length})
              </div>
                ...
                {meal.food_items.map((item) => (
                  ...
                  >
                    ...
                    <div className="text-right">
                      <p className="text-sm font-medium text-gray-900">
                        {formatCalories(Number(item.calories_total) || 0)} cal
                      </p>
                      <p className="text-xs text-gray-500">
 ...
 * Meal history list component
 */
export function MealHistoryList({ meals, title = 'Meal History' }: MealHistoryListProps) {
  ...
  return (
    <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
      <div className="flex items-center justify-between mb-4">
        ...
        <span className="text-sm text-gray-500">{meals.length} meal{meals.length !== 1 ? 's' : ''}</span>
      </div>
      ...
      <div className="flex items-center justify-between p-3 bg-gray-50 rounded-lg mb-4">
        <div className="text-center">
          ...
          <p className="text-sm font-bold text-gray-900">{formatCalories(totalCalories)} cal</p>
        </div>
        <div className="text-center">
        ...
        <div className="text-center">
          <p className="text-xs text-green-500">Fat</p>
